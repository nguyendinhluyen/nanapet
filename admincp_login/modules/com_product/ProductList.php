...
if (($_GET['action'] == 'copy') && isset($_GET['id'])) {
    $sql = "INSERT INTO products (keywords, description, products_key, products_name
                        , products_image, view_count, products_price, product_encourage
                        , price_unit, manufacturer, manufacturer_link, origin
                        , p_type, p_unit, p_attribute, p_color
                        , products_status, products_ordered, products_description, description_ads_vi
                        , product_detail, categories_id, p_new, p_bestsell
                        , p_encourage, language, username, sort_order
                        , recycle_bin, species, status, product_detail_tacdung
                        ...
                        , upgrade_categories_id, products_date_added)
                SELECT keywords, description, CONCAT(products_key, '-', MAX(products_id) + 1), CONCAT('(copy of) ',products_name)
                    , products_image, view_count, products_price, product_encourage
                    , price_unit, manufacturer, manufacturer_link, origin
                    , p_type, p_unit, p_attribute, p_color
                    , products_status, products_ordered, products_description, description_ads_vi
...
for ($i = $begin; $i < $end; ++$i) {
    ...
    if (!empty($img) and ! checkExtentFile($img, $imgExtent)) {
        ...
        $img = ($img == '') ? "<img src ='" . _STYLE_IMG . "picoff.gif'>" : "<a href ='$imgThumb'><img border=0 style=\"cursor:hand\" src=\""
        . _STYLE_IMG . "picon.gif\" onmouseover=\"this.src='" . _STYLE_IMG . "piconover.gif';return overlib('<div><table border=0 cellpadding=2 cellspacing=0><tr><td class=titleImg>Click to Thumbnails</td></tr><tr><td><img src=$imgSrc></a></td></tr></table></div>');\" onmouseout=\"this.src='" . _STYLE_IMG . "picon.gif';return nd();\"></a>";
    }
    ...
    $temp.= $xtemplate->assign_vars($code_cats, array(
        ...
        'products_status' => ($row[$i]['products_status'] == '1') ? $arr_lang['show'] : '<i>' 
                        . $arr_lang['hidden'] . '</i>',
...
$content = $xtemplate->replace($content, array(
    ...
    'oderby'                => $_GET['order'],
    'page'                  => page_div("./?show=product&p=%d_pg&order=" 
